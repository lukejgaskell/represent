    ...
    const timer = setTimeout(async () => {
      try {
        ...
        if (res.district === null || res.district === undefined) {
          errorStore.addError("We were unable to determine your district from your address, please fix your address or type your district manually.")
        } else {
          setDistrict(res.district?.toString())
          setStateAbv(res.state?.toString())
        }
      } catch (e) {
        errorStore.addError("We had a issue determining your district please try again later or type it in manually.")
      }
        ...
        <Grid container direction="column" justifyContent="center" spacing={3}>
          <Grid item container spacing={3}>
            <Grid item xs={12} className="text-center">
              <h2 className="text-xl font-semibold text-gray-800 dark:text-gray-200 mb-2">Help us find your representatives!</h2>
              <p>Please enter your address or your state and congressional district</p>
            </Grid>
            <Grid item xs={12} className="mt-4 mb-4">
              <Grid container spacing={1}>
                ...
                <Grid item xs={6} sm={3}>
                  <TextField fullWidth label="City" variant="outlined" value={city} onChange={event => setCity(event.target.value)} />
                </Grid>
                <Grid item xs={6} sm={2}>
                  <TextField fullWidth label="State" variant="outlined" value={state} onChange={event => setState(event.target.value)} />
                </Grid>
              </Grid>
            </Grid>
          </Grid>
              ...
              <Grid item container direction="column" spacing={1}>
                <Grid item>
                  <TextField
                    ...
                    variant="outlined"
                    value={stateAbv}
                    onChange={event => setStateAbv(event.target.value.toUpperCase())}
                  />
                </Grid>
                <Grid item>
                  <TextField fullWidth label="Congressional District" variant="outlined" value={district} onChange={event => setDistrict(event.target.value)} />
                </Grid>
              </Grid>
            </>
          )}
          <Grid item>
            <Button variant="contained" color="primary" fullWidth disabled={canContinue} onClick={handleContinue}>
              Continue
            </Button>
          </Grid>
        </Grid>
