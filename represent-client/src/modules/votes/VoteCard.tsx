import { Card, CardContent, Divider, Grid, Icon, Typography } from "@material-ui/core"
...
function MemberDisplay({ name, vote_position }: MemberVote) {
  ...
  return (
    <Grid item container key={name} xs={6} sm={3} alignItems="center" direction="row" spacing={1}>
      <Grid item xs={3} className="text-center">
        <VotePositionIcon fontSize="small" style={{ color, marginBottom: "2px" }} />
      </Grid>
      <Grid item xs={9}>
...
export function VoteCard({ result, description, total, date, chamber, question, memberVotes }: IProps) {
      ...
      <CardContent>
        <Grid container direction="column" spacing={1}>
          <Grid item container xs={12} alignItems="center" spacing={4}>
            ...
            <Grid item container direction="column" spacing={0} style={{ lineHeight: "12px" }} xs={6}>
              <Grid item>
          ...
          <Grid item xs={12}>
            <Typography variant="body2" color="textSecondary">{`Voting ${question}`}</Typography>
          </Grid>
          <Grid item container xs={12} justifyContent="space-between" alignItems="center">
            ...
            <Grid item>
              <Typography variant="caption" component="p" color="textSecondary">
                {`${total.yes} Yes / ${total.not_voting + total.present} Abstain / ${total.no} No`}
              </Typography>
            </Grid>
          </Grid>
          <Grid item xs={12}>
